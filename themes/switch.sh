...
if command -v hyprctl >/dev/null; then
    ...
    # A rice with no `cursor` file goes back to the session default rather than
    # keeping the last rice's pointer — otherwise switching away from calamity
    # would leave Terraria cursors behind on every other profile.
    [ -n "$cur_theme" ] || cur_theme="${XCURSOR_THEME:-Adwaita}"
    [ -n "${cur_size:-}" ] || cur_size="${XCURSOR_SIZE:-24}"
    ...
    if [ -z "$found" ]; then
        ...
        # Three places have to agree, because three different things draw a
        # cursor and each looks somewhere else:
        #
        #   hyprctl setcursor   the compositor's own pointer
        #   XCURSOR_THEME       clients that draw their own, at launch
        #   ~/.icons/default    the fallback every toolkit resolves when it
        #                       was given no theme at all
        #
        # Setting only the first two is why a themed cursor still looks
        # default over an already-running window.
        ...
        # Back up a hand-written default once, then own the file. The marker
        # is how we tell our own generated copy from yours.
        def="$HOME/.icons/default/index.theme"
        mkdir -p "$HOME/.icons/default"
        if [ -f "$def" ] && ! grep -q '^# generated by themes/switch.sh' "$def"; then
            cp -f "$def" "$def.bak.$(date +%Y%m%d%H%M%S)"
        fi
        printf '# generated by themes/switch.sh — edit themes/<rice>/cursor instead\n[Icon Theme]\nName=default\nComment=active rice cursor\nInherits=%s\n' \
               "$cur_theme" > "$def"

        # GTK apps ask gsettings before anything else.
        command -v gsettings >/dev/null 2>&1 && \
            gsettings set org.gnome.desktop.interface cursor-theme "$cur_theme" 2>/dev/null || true
    fi
fi
